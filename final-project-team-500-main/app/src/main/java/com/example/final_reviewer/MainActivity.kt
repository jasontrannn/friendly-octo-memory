...
//        setMyReviewsBtn()
    ...
    private fun populateSearch() {
        ...
        //When an item on the recyclerView is clicked
        adapter.onIteamClick = {
            var dialog = RatingFrag()
            dialog.show(supportFragmentManager, "myTAG1")
            Log.i("MY_LOG_TAG", "FROM MainActivity CLASS - FUNC. populateSearch - Clicked an item - position == ${adapter.getPosition()}")
            Log.i("MY_LOG_TAG", "FROM MainActivity CLASS - FUNC. populateSearch - Clicked an item - ${restaurantsList.get(adapter.getPosition()).location.address1}")

            //Get restaurant rating and comment from fragment
            var restComments = dialog.getRatingComments()
            var restRating = dialog.getRatingScore()
            Log.i("MY_LOG_TAG", "FROM MainActivity CLASS - FUNC. populateSearch - Rating == $restRating - Comments == $restComments")

            //Get the restaurant attributes for the selected item on the recyclerView
            var restName = restaurantsList[adapter.getPosition()].name
            var restAddress1 = restaurantsList[adapter.getPosition()].location.address1
            var restCity = restaurantsList[adapter.getPosition()].location.city
            var restZipcode = restaurantsList[adapter.getPosition()].location.zipCode
            var restState = restaurantsList[adapter.getPosition()].location.state

            //Concatenate the full address
            var restFullAddress : String = restAddress1 + ", " + restCity + ", " +
                    restZipcode + ", " + restState

            val db = DBHelper(this)
            //Insert our review into the database
            db.insertData(Restaurant(restName, restFullAddress, restRating, restComments))
        }

...
class PreviousReviews : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter.onItemClick = {
            ...
            dialog.show(supportFragmentManager, "myTAG")
            Log.i("MY_LOG_TAG", "FROM PreviousReviews CLASS - FUNC. onCreate - Clicked an item")
        }

        val bottomNavigationView: BottomNavigationView = findViewById(R.id.bottomNavigationView)
        bottomNavigationView.selectedItemId = R.id.reviews  // Set profile as selected
        ...
        bottomNavigationView.setOnNavigationItemSelectedListener { menuItem ->
            when (menuItem.itemId) {
                ...
                R.id.reviews -> {
                    // Keep this selected or refresh the activity as needed
                    true
                }
                R.id.reviews -> {
                    // Optionally handle navigation to other sections
                    true
                }
                else -> false
            }
    ...
    public fun prevDeleteRow(i : Int) {
        var addressRest = prevArrayList[i].address
        //db.deleteData(addressRest)
